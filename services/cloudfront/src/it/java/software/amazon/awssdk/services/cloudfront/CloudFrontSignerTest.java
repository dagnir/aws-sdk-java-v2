...
import software.amazon.awssdk.services.cloudfront.CloudFrontIntegrationTest;
import software.amazon.awssdk.services.cloudfront.CloudFrontUrlSigner;
...
public class CloudFrontSignerTest {
    ...
    public void testPreSignedUrl() throws Exception {
        ...
        String cannedSignedURL = CloudFrontUrlSigner.getSignedUrlWithCannedPolicy(
                ...
                new File("tst/" + PRIVATE_KEY_FILE), "key", PRIVATE_KEY_ID,
                dateLessThan);
        String cannedSignedURL_der = CloudFrontUrlSigner.getSignedUrlWithCannedPolicy(
                Protocol.https, domainName, new File("tst/"
                        + PRIVATE_KEY_FILE_DER), "key", PRIVATE_KEY_ID,
                dateLessThan);
    ...
    public void testPreSignedCookie() throws Exception {
        ...
        CookiesForCannedPolicy derCookies = CloudFrontCookieSigner.getCookiesForCannedPolicy(
                Protocol.https, domainName,
                new File("tst/" + PRIVATE_KEY_FILE_DER), "key", PRIVATE_KEY_ID,
                dateLessThan);
    ...
    public void makeBytesUrlSafe() {
        ...
        byte[] input = Base64.decode(expectedB64);
        String b64 = Base64.encodeAsString(input);
    ...
    public void buildCustomPolicyForSignedUrl() {
        ...
        String policy = CloudFrontUrlSigner.buildCustomPolicyForSignedUrl("resourcePath", epochDateLessThan, "limitToIpAddressCIDR", epochDateGreaterThan);
    ...
    @Test
    public void getSignedURLWithCannedPolicy() throws InvalidKeySpecException, IOException {
        ...
        Date dateLessThan = new Date(654321000L);
        File pkFile = new File(Thread.currentThread().getContextClassLoader().getResource(pkResource).getFile());
        {
            String signedUrl = CloudFrontUrlSigner.getSignedUrlWithCannedPolicy(Protocol.https, "distributionDomain", pkFile, "s3ObjectKey", "keyPairId", dateLessThan);
            System.out.println("getSignedURLWithCannedPolicy: " + signedUrl);
            assertEquals(expected, signedUrl);
        }
        {
            PrivateKey pk =  loadPrivateKey("tst/pk-APKAJM22QV32R3I2XVIQ.pem");
            String signedUrl = CloudFrontUrlSigner.getSignedUrlWithCannedPolicy("https://distributionDomain/s3ObjectKey", "keyPairId", pk, dateLessThan);
            assertEquals(expected, signedUrl);
        }
    }

    @Test
    public void getSignedURLWithCustomPolicy() throws InvalidKeySpecException, IOException {
        String pkResource= "pk-APKAJM22QV32R3I2XVIQ.pem";
        ...
        File pkFile = new File(Thread.currentThread().getContextClassLoader().getResource(pkResource).getFile());
        {
            String signedUrl = CloudFrontUrlSigner.getSignedUrlWithCustomPolicy(Protocol.https, "distributionDomain", pkFile, "s3ObjectKey", "keyPairId", dateLessThan, dateGreaterThan, ipRange);
            System.out.println("getSignedURLWithCustomPolicy: " + signedUrl);
            assertEquals(expected, signedUrl);
        }
        {
            PrivateKey pk =  loadPrivateKey("tst/pk-APKAJM22QV32R3I2XVIQ.pem");
            String policy = CloudFrontUrlSigner.buildCustomPolicyForSignedUrl("https://distributionDomain/s3ObjectKey", dateLessThan, ipRange, dateGreaterThan);
            String signedUrl = CloudFrontUrlSigner.getSignedUrlWithCustomPolicy("https://distributionDomain/s3ObjectKey", "keyPairId", pk, policy);
