...
public class AwsCodePipelineClientIntegrationTest extends AwsTestBase {
    ...
    public void createFindDelete_ActionType() {
        ActionTypeId actionTypeId = ActionTypeId.builder_().category(ActionCategory.Build).provider("test-provider")
                                                      .version("1").owner(ActionOwner.Custom).build_();
        ArtifactDetails artifactDetails = ArtifactDetails.builder_().maximumCount(1).minimumCount(1).build_();
        client.createCustomActionType(CreateCustomActionTypeRequest.builder_().category(actionTypeId.category())
                                                                         .provider(actionTypeId.provider())
                                                                         .version(actionTypeId.version())
                                                                         .inputArtifactDetails(artifactDetails)
                                                                         .outputArtifactDetails(artifactDetails).build_());
        ...
        client.deleteCustomActionType(DeleteCustomActionTypeRequest.builder_().category(actionTypeId.category())
                                                                         .provider(actionTypeId.provider())
