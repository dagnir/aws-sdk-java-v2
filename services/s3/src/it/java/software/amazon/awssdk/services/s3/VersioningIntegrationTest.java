...
public class VersioningIntegrationTest extends S3IntegrationTestBase {
    ...
    private void createTestData() throws Exception {
        ...
        // Create an initial version of the object we'll create multiple versions
    ...
    protected BucketVersioningConfiguration waitForBucketVersioningStatus(String status) throws InterruptedException {
    ...
    private void gotestGetObject() throws Exception {
        ...
        for (java.util.Iterator iterator = fileContentsByVersionId.keySet().iterator(); iterator.hasNext(); ) {
            ...
            assertStringEqualsStream(fileContentsByVersionId.get(versionId), s3Object.getObjectContent());
        }
    }
    ...
    private void gotestCopyObject() throws Exception {
        // version unaware
        CopyObjectRequest request = new CopyObjectRequest(bucketName, unversionedKey, bucketName, "copiedKey");
        ...
        request = new CopyObjectRequest(bucketName, versionedKey, versionId, bucketName, versionedKey);
        result = s3.copyObject(request);
        assertNotEmpty(result.getETag());
        assertNotNull(result.getLastModifiedDate());
        assertNotEmpty(result.getVersionId());
    ...
    private void gotestPutAcl() throws Exception {
        ...
        s3.setObjectAcl(bucketName, versionedKey, Constants.NULL_VERSION_ID, CannedAccessControlList.PublicReadWrite);
    ...
    private void gotestListVersions() throws Exception {
        ...
        // Test with the delimiter parameter a prefix and returned common prefixes
        versionListing = s3.listVersions(new ListVersionsRequest(bucketName, "aa", null, null, "/", null));
        assertEquals(bucketName, versionListing.getBucketName());
        ...
        versionListing = s3.listVersions(new ListVersionsRequest(bucketName, deletedKey, null, null, null, null));
        assertEquals(bucketName, versionListing.getBucketName());
        assertEquals(0, versionListing.getCommonPrefixes().size());
        assertNull(versionListing.getDelimiter());
        assertNull(versionListing.getKeyMarker());
     ...
     *
     * @param summary
     *            The version summary object to test.
     */
