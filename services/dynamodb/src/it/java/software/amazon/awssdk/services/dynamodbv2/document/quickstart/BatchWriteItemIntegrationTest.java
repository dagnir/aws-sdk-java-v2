 ...
 */
public class BatchWriteItemIntegrationTest extends QuickStartIntegrationTestBase {
    @Before
    ...
    public void howToBatchWrite_ToMultiTables() {
        BatchWriteItemOutcome outcome = dynamo.batchWriteItem(
                ...
                // 2nd table
                new TableWriteItems(UpdateItemIntegrationTest.TABLE_NAME)
    ...
    private void verify_BatchWrite_ToMultiTables() {
        ...
        {   // Verify the 2 items put to the 2nd table via the batch operation
            ItemCollection<?> itemCol = dynamo.getTable(UpdateItemIntegrationTest.TABLE_NAME)
                                              .query(HASH_KEY, 111);
            int count = 0;
            for (Item item : itemCol) {
                System.out.println(item);
