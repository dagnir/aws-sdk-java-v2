...
public class StandardModelFactoriesTest {
    ...
    public void testScalarAttributeStringTimeZone() {
        final Object obj = new AutoKeyAndVal<ZoneId>() {
            @DynamoDbHashKey
            public String getKey() {
                return super.getKey();
            }
            ...
            public void setVal(final ZoneId val) {
                super.setVal(val);
            }
        };
        final DynamoDbMapperTableModel<Object> model = getTable(obj);
        final DynamoDbMapperFieldModel<Object, ZoneId> val = model.field("val");
        assertEquals(DynamoDbAttributeType.S, val.attributeType());
        assertEquals("America/New_York", val.convert(ZoneId.of("America/New_York")).s());
        assertEquals("America/New_York",
                val.unconvert(AttributeValue.builder().s("America/New_York").build())
                        .getId());
    }
    ...
    public void testAutoGeneratedDefaultTimeZone() {
        ...
        final DynamoDbMapperFieldModel<Object, Object> val = model.field("val");
        assertEquals(DynamoDbAutoGenerateStrategy.CREATE, val.getGenerateStrategy());
        assertEquals(ZoneId.of("America/New_York"), val.generate(null));
        assertEquals(ZoneId.of("America/New_York"), val.generate(ZoneId.of("America/Los_Angeles")));
    }
